...
namespace WinMTR.NET
{
    public class HttpHelper
    {
        ...
        /// <param name="nvc">头</param>
        /// <returns>返回请求结果，如果请求失败则返回空字符串</returns>
        public static string HttpPost(string url, string data, string encode, string contentType, NameValueCollection nvc)
        {
            try
            {
                if (string.IsNullOrEmpty(url))
        ...
        public static string HttpPost(string url, string data, string contentType = "application/x-www-form-urlencoded")
        {
            ...
            return HttpPost(url, data, "utf-8", contentType, null);
        }
